...
void handle_request(int client_socket) {
    ...
    if (strncmp(buffer, "GET /hello", 10) == 0) {
    ...
    } else if (strncmp(buffer, "GET /keys/", 10) == 0) {
        char *key = strtok(buffer + 10, " /");
        char *value = get(key);
        if (value == NULL) {
            strcpy(response, "HTTP/1.1 404 Not Found\nContent-Type: text/plain\n\nKey not found!");
        } else {
            sprintf(user_json, "[{\"key\": \"%s\", \"value\": \"%s\"}]", key, value);
            sprintf(response, "HTTP/1.1 200 OK\nContent-Type: application/json\nContent-Length: %ld\r\n\r\n%s",
                    strlen(user_json), user_json);
        }
    } else if (strncmp(buffer, "PUT /keys/", 10) == 0) {
        char *key = strtok(buffer + 10, " /");
        char *body_start = strstr(buffer + 10 + strlen(key) + 1, "\r\n\r\n") + 4;
        char body[100] = {"\0"};
        ...
        for (int i = 0; i < numTokens; i++) {
            if (tokens[i].type == JSMN_STRING &&
                strncmp(body + tokens[i].start, "key", tokens[i].end - tokens[i].start) == 0) {
        ...
        if (strcmp(tokenKey, key) != 0) {
            ...
            sprintf(response,
                    "HTTP/1.1 400 Bad Request\nContent-Type: text/plain\n\nKey, does not match to Ressource!");
            return;
        }
        if (contains(tokenKey) ==
            ...
            sprintf(user_json, "[{\"key\": \"%s\", \"value\": \"%s\", \"status\": \"changed\", \"old_value\": \"%s\"}]",
                    tokenKey, tokenValue, old_);
            sprintf(response, "HTTP/1.1 200 OK\nContent-Type: application/json\nContent-Length: %ld\r\n\r\n%s",
                    strlen(user_json), user_json);
        } else {
            if (put(key, tokenValue) == -1) { //Key und Value werden in die Hashmap gespeichert
                ...
                sprintf(user_json, "[{\"key\": \"%s\", \"value\": \"%s\", \"status\": \"added\"}]", tokenKey,
                        tokenValue);
                sprintf(response,
    ...
    } else if (strncmp(buffer, "DELETE /keys/", 13) == 0) {
        char *key = strtok(buffer + 12, " /");
        sprintf(user_json, "[{\"key\": \"%s\", \"value\": \"%s\", \"status\": \"deleted\"}]", key, get(key));
        delete(key);
        sprintf(response, "HTTP/1.1 200 OK\nContent-Type: application/json\nContent-Length: %ld\r\n\r\n%s",
                strlen(user_json), user_json);
    } else if (strncmp(buffer, "GET", 3) == 0 || strncmp(buffer, "PUT", 3) == 0 || strncmp(buffer, "DELETE", 6) == 0) {
        strcpy(response, "HTTP/1.1" " 404 Not Found\nContent-Type: text/plain\n\nNot found.");
    } else {
        strcpy(response,
...
void run(RestApi *rest_api) {
    ...
    while (true) {
        ...
        int client_socket = accept(rest_api->server_socket, (struct sockaddr *) &client_address,
